<h2 align="center"> Descomplicando os Volumes no K8s. </h3>

<h3 align="center"> 06.1.	O que são volumes? </h3>
...
<h3 align="center"> 06.2.	O StorageClass. </h3>
...
Para criar um StorageClass, crie um arquivo de manifesto **06_storageclass.yml** conforme o exemplo abaixo:  
...
* **Para aplicar essa configuração, execute**: kubectl apply -f 06_storageclass.yml
...
* **provisioner**: qual provisionador será usado. Usar **no-provisioner** indica que esta classe de armazenamento não suporta provisionamento automático.  
...
<h3 align="center"> 06.3.	PV (PersistentVolumes). </h3>
...
**Criando um PV.**  
Para criar um PV, crie o arquivo **06_pv.yml** com o conteúdo abaixo:
...
**Aplique essa configuração criada**: kubectl apply -f 06_pv.yml  
...
<h3 align="center"> 06.4.	PV (PersistentVolumes) com NFS. </h3>
...
**Aplique essa configuração criada**: kubectl apply -f pv-nfs.yaml  
...
<h3 align="center"> 06.5.	PVC (Persistent Volume Claim). </h3>
...
<p align=center><img width="234" height="261" alt="image" src="https://github.com/user-attachments/assets/0635149b-4889-4fca-94d6-c1ba8fab34f1" /></p>

Abaixo, os parâmetros explicados:

* **apiVersion**: indica a versão da API do Kubernetes usada para criar o recurso. A versão v1 é a API estável para criar recursos básicos (Pod, Service, ConfigMap).  
* **kind**: define o tipo de recurso que será criado, nesse caso, PersistentVolumeClaim.  

* **metadata**: informações que serão passadas para o PVC.  
  * **labels**: rótulos usados para organização e seleção.  
  * **pvc: my-pvc**: o rótulo criado.  
  * **name: my-pvc**: nome do PVC.  

* **spec**: especificações de como o PVC deve se comportar.  
  * **accessModes**: define como o volume pode ser montado pelos Pods. Este caso estou usando ReadWriteOnce. Abaixo, os parâmetros possíveis.  
  * **ReadWriteOnce**: o volume pod ser montado como leitura e escrita. Apenas por um nó por vez, mas vários pods no mesmo nó podem usar o volume.  
  * **ReadOnlyMany**: pode ser montado por vários nós, mas é somente leitura.  
  * **ReadWriteMany**: pode ser montado como leitura e escrita e por vários nós ao mesmo tempo.  
  * **ReadWriteOncePod**: leitura e escrita. Apenas um Pod por vez. Introduzido para dar mais segurança contra acesso concorrente.  

* **resources**: nesse caso, são os recursos que o PVC vai solicitar.  
  * **requests**: esse campo indica o mínimo de recurso necessário para que o PVC seja atendido.  
  * **storage**: 1G: o PVC está solicitando 1GiB de armazenamento.  

* **storageClassName**: o nome do StorageClass que você criou.  
* **selector**: é um filtro por labels usado para escolher exatamente qual PersistentVolume (PV) o PVC pode se conectar.  
  * **matchLabels**:  
  * **storage**: nfs: qual PV será usado. Esse label deve ser igual ao label adicionado no PV criado anteriormente.  

**Para aplicar essas configurações criadas:** kubectl apply -f pvc.yaml  
**Para visualizar os PVCs criados**: kubectl get pvc  
<p align=center> <img width="600" height="36" alt="image" src="https://github.com/user-attachments/assets/628b5bc5-afba-42d9-b2f3-31e90eacfff1" /></p>

No status acima, o mesmo fica com Status **Pending** pois ele está aguardando por algum “consumidor” para utilizar esse PVC como volume.

Para ver detalhes do PVC criado, execute **kubectl describe pvc my-pvc**
<p align=center><img width="1120" height="260" alt="image" src="https://github.com/user-attachments/assets/3f38825b-f4b1-4c33-b725-561a5e6d6f7b" /></p>

**Criando um deployment para usar esse PVC.**  
Crie o arquivo **redis-deployment.yml** com o conteúdo abaixo, para criar um deployment para utilizar esse PVC.
<p align=center><img width="258" height="449" alt="image" src="https://github.com/user-attachments/assets/f95cf6c8-9c17-476e-b8ea-2efe1d0fc316" /></p>

* **Para aplicar esse Deployment:** kubectl apply -f redis-deployment.yml  

Após aplicar essa configuração, execute novamente **kubectl describe pvc my-pvc**. Agora, o PVC já se encontra como **Bound**.
<p align=center><img width="571" height="38" alt="image" src="https://github.com/user-attachments/assets/c792ab21-a81a-4bb8-bdad-4a4e380000a9" /></p>

**Criando um Deployment do NGINX.**  
Nesta aula, será criado o arquivo **nginx-deployment.yml** para criar um Pod para usar esse PVC.
<p align=center><img width="324" height="282" alt="image" src="https://github.com/user-attachments/assets/c7aa7405-bcf5-441b-b210-a36cc0acad10" /></p>

Antes dessa nova implantação, delete o **PVC** e o **PV** com **NFS**:  
* kubectl delete pvc my-pvc
* kubectl delete pv my-pv-nfs

Crie novamente o pv e o pvc.
* kubectl apply -f pv.yml
* kubectl apply -f pv-nfs.yml
* kubectl apply -f pvc.yml

Aplique a configuração criada no **nginx-deployment.yml**: k apply -f **nginx-deployment.yml**

Após esses procedimentos, o Pod foi criado e se encontra utilizando o PVC.
<p align=center><img width="570" height="48" alt="image" src="https://github.com/user-attachments/assets/0c7857db-af5c-447c-8efc-b6c248bf1c6f" /></p>

Em um resumo simples sobre StorageClass, Persistent Volume e Persistent Volume Claim.  
<p align=center><img width="570" height="104" alt="image" src="https://github.com/user-attachments/assets/e50d57e4-de7e-4d69-bc0f-7a3838da979a" /></p>
