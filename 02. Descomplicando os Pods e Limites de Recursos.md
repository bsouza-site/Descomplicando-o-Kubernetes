...
Ao executar o comando **kubectl get pods -o wide**, são apresentadas informações adicionais dos pods.  
<p align="center"><img width="869" height="35" alt="image" src="https://github.com/user-attachments/assets/8c41bb23-9923-4f3a-aa6a-6bda3b41989c" /></p>

As informações acima indicam:  
* **NAME**: nome do Pod. É o identificador único dentro do namespace.
* **READY**: quantos containers estão prontos e o total de containers.
* **STATUS**: estado atual do Pod. Os principais estados podem ser:
  * **Running**: rodando normalmente.
  * **Pending**: aguardando agendamento.
  * **CrashLoopBackOff**: falhando e reiniciando.
  * **Completed**: finalizado com sucesso.
* **RESTARTS**: quantas vezes os containers reiniciaram.
* **AGE**: tempo desde a criação do Pod.
* **IP**: IP interno do Pod.
* **NODE**: node em que o Pod está executando.
* **NOMINATED NODE**: node "pré-selecionado" pelo scheduler.
* **READINESS GATES**: condições extras para considerar o Pod pronto.
...
**kubectl cp**  
O comando **kubectl cp** é utilizado para copiar arquivos para um container e do container para o host local, para isso, utilize os comandos abaixo:  

* **Copiar o arquivo do host local para o container**: kubectl cp index.html testes/nginx:/usr/share/nginx/html  
* **Copiar do container para o host local**: kubectl cp testes/nginx:/usr/share/nginx/html/index.html ./index.html  
...
**Para aplicar este manifesto**: kubectl apply -f pod_2.yaml
...
* **Ver as labels de um namespace**: kubectl get ns **--show-labels**  
...
<h3 align="center"> 02.8.	Namespace em Terminating. </h3>

No Kubernetes, um namespace fica preso em um estado de **Terminating** quando o cluster não consegue excluir o namespace porque recursos ou finalizadores dependentes ainda estão presentes. Os finalizadores são mecanismos de segurança que impedem a exclusão de um recurso até que tarefas específicas de limpeza (como desanexar o armazenamento ou limpar serviços externos) sejam concluídas.  
<p align="center"><img width="410" height="36" alt="image" src="https://github.com/user-attachments/assets/1478fbfc-38d5-4ede-8cef-a27cb2e3b916" /></p>

**Como resolver namespace em estado Terminating?**  
Para resolver um namespace preso no estado Terminating, execute os passos abaixo:  

* **Exporte o namespace para JSON**: kubectl get ns **namespace** -o json > **ns.json**  
* Acesse esse JSON, e no campo **spec**, remova qualquer referência em **finalizers**:  
<p align="center"><img width="187" height="52" alt="image" src="https://github.com/user-attachments/assets/188ad9f1-6140-411f-88fa-097181f80ccf" /></p>

* **Aplique via API**: kubectl replace --raw "/api/v1/namespaces/**namespace**/finalize" -f **ns.json**  

Após isso, o namespace foi removido.
